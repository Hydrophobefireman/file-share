...
async def set_socket_id():
    ...
    return _response({"type": "init_connection", "sess_id": sess_id})
...
async def validate_socket_id():
    data: dict = await request.get_json()
    sess_id: str = data.get("sess_id")
    session["u_id"] = token_hex(15)
    peer = get_peer_socket(sess_id, session["u_id"])
    if peer:
        return _response({"type": "existing_connection"})
    return _response({"type": "unique"})
...
async def get_socket_id():
    ...
    sess_id: str = data.get("sess_id")
    session["sess_id"] = sess_id
    peer = get_peer_socket(sess_id, session["u_id"], True)
    if not peer:
        return _response({"type": "no_id"})
    init_conn: list = []
    init_conn.extend(i.device_id for i in peer)
    return _response({"init_conn": init_conn})


def _response(
    jsobj: dict, headers: dict = {"content-type": DEFAULT_CONTENT_TYPE}, code: int = 200
) -> Response:
    return Response(json.dumps(jsobj), headers=headers, status=code)
