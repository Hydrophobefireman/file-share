...
export class FileDownloadElement extends HTMLElement {
  ...
  constructor(url, metaData) {
    super();
    const innerHTML = `${css}<div full><div area><div header></div><div download></div><div size></div><span cancel>Cancel</span><span download><a>Download</a></span></div></div>`;
    this._url = url;
    const template = document.createElement("template");
    template.innerHTML = innerHTML;
    const shadowRoot = this.attachShadow({ mode: "open" });
    ...
    this.dlButton = shadowRoot.querySelector("a");
